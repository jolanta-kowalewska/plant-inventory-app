...
table_tasks = dynamodb.Table(os.environ['DYNAMODB_TABLE_GARDEN_TASKS'])
...
def lambda_handler(event, context):
    ...
    response = ssm.get_parameters(
        Names=[
            ...
            os.environ['OPENWEATHER_API_KEY_PATH']
        ],
        WithDecryption=True
    )

    params = {p['Name']: p['Value'] for p in response['Parameters']}
    anthropic_api_key = params[os.environ['ANTHROPIC_API_KEY_PATH']]
    weather_api_key = params[os.environ['OPENWEATHER_API_KEY_PATH']]
...
def get_tasks(user_id, month):
    ...
    tasks = [t for t in response['Items'] 
            if t['date'].startswith(f"{current_year}-{next_month}")]
